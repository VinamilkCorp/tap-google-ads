        ...
        if mdata["breadcrumb"]:
            ...
            if (
                ...
                selected_fields.update(
                    mdata["metadata"]["tap-google-ads.api-field-names"]
                )
...
def write_bookmark_for_core_streams(state, stream, customer_id, last_pk_fetched):
    ...
    singer.write_bookmark(
        state, stream, customer_id, {"last_pk_fetched": last_pk_fetched}
    )
...
class BaseStream:  # pylint: disable=too-many-instance-attributes
    ...
    def create_full_schema(self, resource_schema):
        ...
        self.resource_fields = filter_out_non_attribute_fields(
            self.resource_object["fields"]
        )
        self.full_schema = create_nested_resource_schema(
            resource_schema, self.resource_fields
        )
    ...
    def format_field_names(self):
        ...
        for resource_name, schema in self.full_schema["properties"].items():
            ...
            if resource_name == "ad_group_ad":
                for ad_field_name, ad_field_schema in self.full_schema["properties"][
                    "ad_group_ad"
                ]["properties"]["ad"]["properties"].items():
            ...
            if (
                ...
                self.stream_schema["properties"][resource_name + "_id"] = schema[
                    "properties"
                ]["id"]
    ...
    def sync(
        ...
        state = singer.set_currently_syncing(
            state, [stream_name, customer["customerId"]]
        )
        singer.write_state(state)

        # last run was interrupted if there is a bookmark available for core streams.
        last_pk_fetched = (
            singer.get_bookmark(state, stream["tap_stream_id"], customer["customerId"])
            or {}
        )
        ...
        with metrics.record_counter(stream_name) as counter:
            ...
            while is_more_records:
                ...
                if (
                    record
                    and self.filter_param
                    and stream_name not in limit_not_possible
                ):
...
class UserInterestStream(BaseStream):
    ...
    def format_field_names(self):
        ...
        self.stream_schema["properties"]["id"] = schema["properties"][
            "user_interest_id"
        ]
...
class ReportStream(BaseStream):
        ...
        for resource_name, schema in self.full_schema["properties"].items():
            for field_name, data_type in schema["properties"].items():
                ...
                if resource_name == "ad_group_ad" and field_name == "ad":
                    for ad_field_name, ad_field_schema in data_type[
                        "properties"
                    ].items():
                        self.stream_schema["properties"][
                            ad_field_name
                        ] = ad_field_schema
                elif resource_name not in {"metrics", "segments"}:
                    self.stream_schema["properties"][
                        f"{resource_name}_{field_name}"
                    ] = data_type
                else:
    ...
    def build_stream_metadata(self):
        ...
        for report_field in self.fields:
            # Transform the field name to match the schema
            is_metric_or_segment = report_field.startswith(
                "metrics."
            ) or report_field.startswith("segments.")
    ...
    def sync(self, sdk_client, customer, stream, config, state, query_limit):
        ...
        conversion_window_date = (
            utils.now().replace(hour=0, minute=0, second=0, microsecond=0)
            - conversion_window
        )
        ...
        while query_date <= end_date:
            ...
            with Transformer() as transformer:
                ...
                for message in response:
                    ...
                    transformed_message = self.transform_keys(json_message)
                    record = transformer.transform(
                        transformed_message, stream["schema"]
                    )
